 ...
 */
async function gatherSkillsContent(skillPaths: string[]): Promise<string> {
  const parts: string[] = [];
 ...
 */
export async function analyzeSemanticChains(
  skillPaths: string[],
  ...
  try {
    ...
    const analysis: SemanticChainAnalysis = JSON.parse(jsonMatch[0]);
