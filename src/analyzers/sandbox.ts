...
import type { Finding, Severity, SandboxAnalyzer } from '../types.js';
 ...
 */
function getHoneypotSetupCommands(): string[] {
  return HONEYPOT_FILES.map(h => 
    `mkdir -p $(dirname ${h.path}) && echo '${h.content}' > ${h.path}`
  );
}
