...
// Pattern data loaded from YAML
interface DepsPatterns {
  malicious_npm: Array<{ name: string; versions: string[]; reason: string }>;
  malicious_pip: Array<{ name: string; versions: string[]; reason: string }>;
  malicious_go: Array<{ name: string; versions: string[]; reason: string }>;
  popular_npm: string[];
  popular_pip: string[];
  popular_go: string[];
}
...
let patternsCache: DepsPatterns | null = null;

/**
 * Load dependency patterns from YAML
 */
export async function loadPatterns(): Promise<DepsPatterns> {
  ...
  patternsCache = parseYaml(content) as DepsPatterns;
 ...
 */
export async function parsePackageJson(path: string): Promise<Dependency[]> {
 ...
 */
export async function parseRequirementsTxt(path: string): Promise<Dependency[]> {
  const content = await readFile(path, 'utf-8');
  const deps: Dependency[] = [];
 ...
 */
export async function parseGoMod(path: string): Promise<Dependency[]> {
  const content = await readFile(path, 'utf-8');
  const deps: Dependency[] = [];
 ...
 */
function versionMatches(version: string, patterns: string[]): boolean {
...
async function checkNpmDeps(
  ...
  for (const dep of deps) {
    ...
    if (checkRegistry) {
      ...
      const info = result.info as NpmPackageInfo;
...
async function checkPipDeps(
  ...
  for (const dep of deps) {
    ...
    if (checkRegistry) {
      ...
      const info = result.info as PypiPackageInfo;
 ...
 */
async function checkPackageJsonScripts(path: string): Promise<Finding[]> {
  const findings: Finding[] = [];
...
export class DepsAnalyzer implements Analyzer {
  name = 'deps';
  private checkRegistry: boolean;
  
  constructor(options: { checkRegistry?: boolean } = {}) {
...
export function createDependencyAnalyzer(options: { checkRegistry?: boolean } = {}) {
