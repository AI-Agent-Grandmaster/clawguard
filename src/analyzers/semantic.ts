...
import type { Finding, Severity, Analyzer } from '../types.js';
...
interface SemanticAnalysis {
  ...
  summary: {
    ...
    purpose_capability_mismatch: boolean;
 ...
 */
function createLocalProvider(model: string, baseUrl: string = 'http://localhost:11434'): LLMProvider {
  return {
...
export class SemanticAnalyzer implements Analyzer {
  ...
  private async gatherSkillContent(skillPath: string): Promise<string> {
    const parts: string[] = [];
  ...
  private parseAnalysis(text: string): SemanticAnalysis {
    try {
  ...
  private convertToFindings(analysis: SemanticAnalysis, skillPath: string): Finding[] {
