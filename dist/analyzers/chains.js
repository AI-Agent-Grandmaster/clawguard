 ...
 */
async function analyzeSkillCapabilities(skillPath) {
 ...
 */
function findDangerousChains(skills) {
    const findings = [];
    ...
    const outboundSkills = skills.filter(s => s.capabilities.has('network_out') ||
 ...
 */
export async function analyzeAttackChains(skillPaths) {
    // Analyze each skill
    const skillAnalyses = await Promise.all(skillPaths.map(path => analyzeSkillCapabilities(path)));
 ...
 */
export async function generateCapabilityReport(skillPaths) {
    const skills = await Promise.all(skillPaths.map(path => analyzeSkillCapabilities(path)));
